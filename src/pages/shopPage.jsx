...
// بيانات المنتجات (وضعتها خارج المكون لأنها ثابتة)
const products = [
  {
    id: 1,
    name: "Elegant Blazer",
...
const ShopPage = () => {
  // State للتحكم في واجهة المستخدم
  const [isCartVisible, setIsCartVisible] = useState(false);
  const [isMobileFilterVisible, setIsMobileFilterVisible] = useState(false);
  const [isToastVisible, setIsToastVisible] = useState(false);
  const [viewMode, setViewMode] = useState("grid"); // 'grid' or 'list'

  // State لإدارة الفلاتر (للجزء المرئي فقط كما في الكود الأصلي)
  const [selectedSize, setSelectedSize] = useState("M");
  const [selectedColor, setSelectedColor] = useState(null);
  const [selectedTags, setSelectedTags] = useState([]);

  // useEffect للتحكم في إخفاء التنبيه (toast) بعد فترة
  useEffect(() => {
    if (isToastVisible) {
  ...
  // useEffect للتحكم في سكرول الصفحة عند فتح القوائم الجانبية
  useEffect(() => {
      ...
      <div className="shop-container">
        ...
        <aside className="sidebar-filters" id="sidebar-filters">
          <div className="filter-header">
            <h3>Filters</h3>
            {/* زر الفلتر للموبايل سيكون في الهيدر الرئيسي للمنتجات */}
          </div>
          {renderFilters()}
        </aside>
      ...
      >
        <div className="mini-cart-sidebar" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mini-cart-items">
            {/* محتوى السلة يضاف هنا ديناميكيا */}
